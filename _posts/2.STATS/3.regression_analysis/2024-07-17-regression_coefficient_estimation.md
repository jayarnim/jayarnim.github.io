...
- `A.1` 반응변수와 설명변수의 선형성 가정
    ...
    $$
    \mathbb{E}\left[Y\mid X\right]=\beta^{T}X
    $$

- `A.2` 오차항의 자기상관 없음(No Autocorrelation) 가정

    $$
    \mathrm{Cov}\left[\varepsilon_i, \varepsilon_j\mid X\right]=0,\quad i\ne j
    $$

- `A.3` 오차항과 설명변수 간 조건부 독립 가정

    $$
    \mathbb{E}\left[\varepsilon_i \mid X\right]=0,\quad \forall i=1,\cdots,n
    $$

...
- **BPR(`B`ayesian `P`ersonalized `R`anking)** : Probabilistc Pairwise Ranking Objective Function
...
- **Convert to Probability**
    ...
    - Change of Variables:
        ...
        \therefore \text{d}x
        &= \frac{\text{d}f(t)}{\text{d}t} \text{d}t
        \end{aligned}$$
    ...
    - $f(t), g(t)$ is related to CDF(`C`umulative `D`istribution `F`unction):
        ...
        f(t)
        &= p\left(\hat{z} > t\mid z=\text{NEG}\right)\\
        &= 1 - p\left(\hat{z} \le t\mid z=\text{NEG}\right)\\
        \\
        \therefore \text{d}x
        &= \frac{\text{d}f(t)}{\text{d}t} \text{d}t\\

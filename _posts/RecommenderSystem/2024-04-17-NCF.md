...
- **DELF(`D`ual-`E`mbedding based `L`atent `F`actor Model)**
    - **Interaction based Embedding** : 사용자와 아이템의 고유한 정체성, 예컨대 사용자의 독립적인 선호나 아이템의 독립적 특성보다는 과거 어떤 아이템, 사용자와 상호작용했는지에 지나치게 의존함
    - **ID based Embedding** : 사용자, 아이템의 ID만을 활용하므로 사용자 행동 패턴이나 아이템 특성을 유추할 만한 맥락 정보가 부족함
...
- **Interaction based Embedding**
    ...
    $$\begin{aligned}
    \mathbf{P}_{i}&=\text{Embedding}\left[\mathbf{R}_{i,:}\right] \in \mathbb{R}^{K}\\
    \mathbf{Q}_{j}&=\text{Embedding}\left[\mathbf{R}_{:,j}\right] \in \mathbb{R}^{K}
    \end{aligned}$$

    - $\mathbf{R}_{i,:}$ : Multi-Hot Vector of User $i$ @ User-Item Interaction Matrix
    - $\mathbf{R}_{:,j}$ : Multi-Hot Vector of Item $j$ @ User-Item Interaction Matrix
    - $\mathbf{P}_{i}$ : ID Embedding Vector of User $i$
    - $\mathbf{Q}_{j}$ : ID Embedding Vector of Item $j$

- **ID based Embedding**

    $$\begin{aligned}
    \mathbf{U}_{i}&=\text{Embedding}\left[\mathbf{Z}_{i}\right] \in \mathbb{R}^{K}\\
    \mathbf{V}_{j}&=\text{Embedding}\left[\mathbf{Z}_{j}\right] \in \mathbb{R}^{K}
    \end{aligned}$$

    - $\mathbf{Z}_{i}$ : ID One-Hot Vector of User $i$
    - $\mathbf{Z}_{j}$ : ID One-Hot Vector of Item $j$
    - $\mathbf{U}_{i}$ : ID Embedding Vector of User $i$
    - $\mathbf{V}_{j}$ : ID Embedding Vector of Item $j$

- **Embedding Combination**
    - User Embedding Combination:

        $$\begin{aligned}
        \mathbf{X}_{i}&=\mathbf{P}_{i} \oplus \mathbf{U}_{i} \in \mathbb{R}^{2K}
        \end{aligned}$$

    - Item Embedding Combination:

        $$\begin{aligned}
        \mathbf{Y}_{j}&=\mathbf{Q}_{j} \oplus \mathbf{V}_{j} \in \mathbb{R}^{2K}
        \end{aligned}$$

- **Neural Collaborative Filtering and Predict**

    $$\begin{aligned}
    \hat{r}_{u,i} &= \sigma\Big[\text{MLP}_{\text{ReLU}}\left[\mathbf{X}_{i} \oplus \mathbf{Y}_{j}\right]\Big]
    \end{aligned}$$
...
- **MeFL(`Me`tric `F`unction `L`earning)** : 사용자, 아이템 표현 학습 시 삼각 부등식을 고려함으로써 합리적 추천 결과를 도모함
    ...
    - Embedding:
        ...
        $$\begin{aligned}
        \mathbf{P}^{\text{MeFL}}&= \text{Embedding}\left[\mathbf{Y}\right] \in \mathbb{R}^{M \times K}\\
        \mathbf{Q}^{\text{MeFL}}&= \text{Embedding}\left[\mathbf{Y}^{T}\right] \in \mathbb{R}^{N \times K}
        \end{aligned}$$
...
- **MaFL(`Ma`tching `F`unction `L`earning)**
    ...
    - Embedding:
        ...
        $$\begin{aligned}
        \mathbf{P}^{\text{MaFL}}&= \text{Embedding}\left[\mathbf{R}\right] \in \mathbb{R}^{M \times K}\\
        \mathbf{Q}^{\text{MaFL}}&= \text{Embedding}\left[\mathbf{R}^{T}\right] \in \mathbb{R}^{N \times K}
        \end{aligned}$$

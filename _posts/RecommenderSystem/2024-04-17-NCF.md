...
| Type | How to | Pos | Neg |
|---|---|---|---|
| ID Embedding | 사용자, 아이템 ID 임베딩 | 사용자, 아이템의 고유한 특징 포착 가능 | 맥락 정보 부재로 인해 최적화 어려움 |
| History Embedding | 사용자-아이템 상호작용 행렬 선형 변환 | 사용자 행동 패턴, 아이템 구매 패턴 포착 가능 | 상호작용에 지나치게 의존하여 고유한 특징 포착 어려움 |

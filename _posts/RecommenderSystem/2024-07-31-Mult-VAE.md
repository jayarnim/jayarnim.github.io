...
- $\overrightarrow{\mathbf{z}} \mid \overrightarrow{\mathbf{x}};\Theta \sim P$ 의 근사 분포 $\overrightarrow{\mathbf{z}} \mid \Phi \sim Q$ 정의:

    $$\begin{aligned}
    q\left(\overrightarrow{\mathbf{z}} \mid \Phi\right)
    &= \mathcal{N}\left(\mu(\overrightarrow{\mathbf{x}} \mid \Phi), \text{diag}[\sigma^2(\overrightarrow{\mathbf{x}} \mid \Phi)]\right)\\
    &\approx \mu(\overrightarrow{\mathbf{x}} \mid \Phi) + \sigma(\overrightarrow{\mathbf{x}} \mid \Phi) \odot \epsilon \quad \text{for} \quad \epsilon \sim \mathcal{N}\left(0, \mathbf{I}_{K}\right)
    \end{aligned}$$
...
- ELBO:
    ...
    $$
    \text{ELBO}
    = \underbrace{\mathbb{E}_{\mathbf{z} \mid \Phi \sim Q}\Big[\log{\mathcal{L}\left(\overrightarrow{\mathbf{x}} \mid \overrightarrow{\mathbf{z}} ; \Theta \right)}\Big]}_{-\text{Reconstruction Loss}} - \beta \cdot \underbrace{D_{KL}\Big[q\left(\overrightarrow{\mathbf{z}} \mid \Phi\right) \parallel \pi(\overrightarrow{\mathbf{z}})\Big]}_{\text{KL Divergence}}
    $$
...
- $$\overrightarrow{\mathbf{z}}_{u} \mid \overrightarrow{\mathbf{x}}_{u} ; \Theta \sim P$$ 의 근사 분포 $$\overrightarrow{\mathbf{z}}_{u} \mid \Phi \sim Q$$ 정의:

    $$\begin{aligned}
    q\left(\overrightarrow{\mathbf{z}}_{u} \mid \Phi\right)
    &= \mathcal{N}\left(\mu(\overrightarrow{\mathbf{x}}_{u} \mid \Phi), \text{diag}[\sigma^2(\overrightarrow{\mathbf{x}}_{u} \mid \Phi)]\right)\\
    &\approx \mu(\overrightarrow{\mathbf{x}}_{u} \mid \Phi) + \sigma(\overrightarrow{\mathbf{x}}_{u} \mid \Phi) \odot \epsilon \quad \text{for} \quad \epsilon \sim \mathcal{N}\left(0, \mathbf{I}_{K}\right)
    \end{aligned}$$
...
- ELBO:
    ...
    $$
    \text{ELBO}
    = \underbrace{\mathbb{E}_{\mathbf{z}_{u} \mid \Phi \sim Q}\Big[\log{\mathcal{L}\left(\overrightarrow{\mathbf{x}}_{u} \mid \overrightarrow{\mathbf{z}}_{u} ; \Theta \right)}\Big]}_{-\text{Reconstruction Loss}} - \beta \cdot \underbrace{D_{KL}\Big[q\left(\overrightarrow{\mathbf{z}}_{u} \mid \Phi\right) \parallel \pi(\overrightarrow{\mathbf{z}}_{u})\Big]}_{\text{KL Divergence}}
    $$

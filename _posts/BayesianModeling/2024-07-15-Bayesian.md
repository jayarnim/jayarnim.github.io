...
- **베이즈 정리(Bayes' Theorem)** : 모수의 사후 확률 분포를 추정하는 방법
    ...
    - $$p\left(\mathcal{D} \mid \theta \right)$$ : 우도(Likelihood)로서, 데이터에 대한 모수의 상대적 적합성으로 해석되며, 표본의 변동성에 의한 우발적 불확실성을 반영함
    - $$p\left(\theta\right)$$ : 사전 확률 분포(Prior Probability Distribution)로서, 주어진 정보의 한계에 의한 인식 불확실성을 반영함

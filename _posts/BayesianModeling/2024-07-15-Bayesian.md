...
- ***신념이 주관적 체계를 넘어 객관적 체계로서 정당화될 수 있는가?***
    - 사전 확률 제약하기(Constraining Priors)
    - 사전 확률 씻겨내기(Washing Out Priors)

## Condition
-----

> 윤리적으로 중립인 명제에 대하여 그것의 참 여부에 따라 $0-1$ 사이의 서수적 효용을 획득하거나 상실할 수 있는 내기 상황을 가정하자. $X$ 에 대한 행위자의 신념도가 $p$ 이라는 것은 그 행위자는 $X$ 이면 단위 $1$ 을 지불하고 그렇지 않은 경우에는 $0$ 을 지불하는 도박에 대해 $p$ 를 지불할 용의가 있다는 것이다(de Finetti, 1937, p.62). <br> <br> 이처럼 베이즈주의는 개인의 신념도를 행위의 자발성과 동일시하는 행동주의적 접근을 취한다(이영의, 2015, p.35). 즉, **내기 상황 하에서 신념과 선호를 기대 효용의 형태로 연결하고, 이를 최적화 혹은 효용 극대화라는 선택으로써 측정 가능한 형태로 이끌어낸다.**
...
- **역추리 문제(Inverse Inference Problem)** : 다양한 원인 중 하나로부터 발생했음이 틀림없는 한 가지 사건이 발생했다고 가정하고, 그 원인들이 존재할 각각의 확률을 추리하는 문제(이영의, 2015, p.62)
    ...
    - **직접 확률(Direct Probability)** : 시행의 성질이 먼저 규정되고 이어서 시행에서 하나 또는 그 이상의 가능한 결과가 발생할 확률(이영의, 2015, p.62)
    ...
    - **역확률(Inverse Probability)** : 이미 시행의 결과가 제시되고 그 다음 실제로 실행된 시행이 가능한 시행 중 특정 시행에 해당할 확률(이영의, 2015, p.62)
...
- **당구대의 비유(Bayes’s Billiard Table Analogy)**
...
- **예증(Scholium)**
    ...
    > 내가 $Z$ 라고 불렀던 그러한 사건의 경우에, 일정 횟수로 시행하고 그것이 발생하고 실패한 횟수를 통해, 그것에 대해서는 다른 어떤 것도 알지 못한 상태에서 그것의 확률에 관해 추측할 수 있고, 언급한 명적들의 크기를 계산하는 일반적인 방법에 의해 그러한 추측이 옳을 기회를 알 수 있다. <br> <br> 그리고 동일한 규칙이 그것에 관해 이루어진 어떤 시행에서도, 우리가 사전에 전혀 알지 못하는 확률을 갖는 사건의 경우에도 사용될 수 있다는 것은 다음을 생각해보면 나타난 것 같다. 즉, 어떤 횟수의 시행에서 그것이 다른 횟수보다 어떤 가능한 다른 횟수로 발생할 것이라는 점을 생각할 이유가 없다. <br> <br> 그러므로 나는 다음부터 당연히 사건 $Z$ 에 대해 주어진 규칙이 또한 어떠한 시행이 이루어지거나 관찰되기 전에 어떤 것도 알려지지 않은 사건의 확률에도 이용될 수 있다고 간주할 것이다. 나는 그러한 사건을 알려지지 않은 사건이라고 부를 것이다. (Bayes, 1763, p.393)
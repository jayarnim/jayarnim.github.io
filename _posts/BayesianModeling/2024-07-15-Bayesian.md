...
- **베이즈 정리(Bayes' Theorem)** : 모수의 사후 확률 분포를 추정하는 방법
    ...
    - $$p\left(\theta \mid \mathcal{D} \right)$$ : 사후 확률 분포(Posterior Probability Distribution)로서, 모수의 불확실성을 나타냄
    - $$p\left(\mathcal{D} \mid \theta \right)$$ : 우도(Likelihood)로서, 데이터에 대한 모수의 상대적 적합성으로 해석되며, 표본 변동성에서 비롯한 특정 모수 하에서의 우발적 불확실성을 반영함
    - $$p\left(\theta\right)$$ : 사전 확률 분포(Prior Probability Distribution)로서, 주어진 정보의 한계에서 비롯한 모수에 대한 인식 불확실성을 반영함
    - $$p\left(\mathcal{D}\right)$$ : 데이터가 관찰될 확률로서, 모수에 대한 신념을 뒷받침하는 증거(Evidence)로 해석됨

...
- **모수(Parameter)** : 데이터 생성 과정의 근본적인 원리를 설명하는 상수
	...
	- **베이지안(Bayesian)**: 
		...
		> 모수는 객관적이고 절대적인 진리일 수 있다. 그러나 인간의 불완전한 지식 체계에서는 그 값을 정확히 알거나 확실하게 추정할 수 없다. 따라서 **모수는 확률로 표현되어야 하며,** 이 표현은 주어진 지식 체계 하에서 관측된 데이터에 근거했을 때 모수가 어떠할 것이라는 믿음을 나타낸다. 모수를 추정할 때는 데이터가 내재적으로 가지는 불확실성(Aleatoric Uncertainty)뿐만 아니라, 기존 지식이 가지는 불확실성(Epistemic Uncertainty)까지 함께 모델링해야 한다.
...
- **If the sample size is not large enough,**
    ...
    - **베이지안(Bayesian)** : 모수에 대한 믿음의 불확실성을 확률로 표현함
...
- **모듈의 이해**
    - **사전확률분포(Prior Probability Distribution)** : 사전 지식의 불확실성을 모델링하는 모듈로서, 사전 정보를 기초로 설정된 초기 믿음

        $$
        \theta \sim P
        $$

    - **우도(Likelihood or Likelihood Function)** : 데이터의 불확실성을 모델링하는 모듈로서, 모수가 $\theta$ 로 주어졌을 때, 관측치 $\mathcal{D}$ 가 실현될 가능성

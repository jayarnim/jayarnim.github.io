...
- **불확실성(Uncertainty)**: 어떠한 사건에 대하여 확신할 수 없는 상태
    ...
    - **인식적 불확실성(Epistemic Uncertainty)**: 정보 부족으로 인해 발생하는 불확실성
        - **파라미터 불확실성(Parameter Uncertainty)**: 관측치 부족으로 인하여 주어진 모형 안에서 최적 파라미터를 확신할 수 없는 문제
        - **구조적 불확실성(Structural Uncertainty)**: 데이터 생성 메커니즘을 기술하는 함수 구조를 확신할 수 없어 특정 모형을 전제할 수 없는 문제
        - **표현의 불확실성(Latent Uncertainty)**: 잠재공간의 비식별성으로 인해 치역만으로 정의역(latent representation)을 유일하게 결정할 수 없는 문제
...
- **`MC Dropout`(`M`onte-`C`arlo Dropout)**: 드롭아웃이 적용된 인공신경망의 가우시안 프로세스의 유한 차원 근사임을 증명하여 결정론적 신경망과 동일한 비용으로 불확실성 모델링이 가능함을 확인함
    ...
    ![01](/_post_refer_img/BayesianModeling/3.Application/08-01.png){: width="100%"}

...
- **커널 함수(Kernel Function)** : 두 입력 벡터 간 유사도 측정 함수로서 공분산 행렬을 구성함
    ...
    | Sigmoid | $$\text{tanh}\left(\alpha \cdot X \cdot X^{\prime} + \beta\right)$$ |
    ...
    | Periodic | $$\exp\left[-2 \sin^2\left( \displaystyle\frac{\pi \vert X-X^{\prime} \vert}{p} \right) \Bigg/ \ell^2 \right]$$ |

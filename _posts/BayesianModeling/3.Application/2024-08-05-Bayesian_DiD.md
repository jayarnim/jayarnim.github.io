...
- policy activation uncertainty (processing time per unit is the same):
    ...
    $$\begin{aligned}
    T_{t} \mid \phi_{t} &\sim \mathrm{Bernoulli}(\phi_{t})\\
    \phi_{t} &\overset{\mathrm{i.i.d}}{\sim} \mathrm{Beta}(1,1)
    \end{aligned}$$
...
- posterior estimation:
    ...
    $$\begin{aligned}
    &p(\alpha_{i},\gamma_{t},\delta_{i},\sigma,\pi_{i},\phi_{t} \mid Y_{i,t}, D_{i}, T_{t})\\
    ...
    &\quad\times \underbrace{p(T_{t}\mid\phi_{t})\cdot p(\phi_{t})}_{\propto p(\phi_{t}\mid T_{t})}
    \end{aligned}$$
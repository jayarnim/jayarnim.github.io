...
- **부트스트랩(Bootstrap)** : 전체 데이터 세트를 $K$ 개의 데이터 블록으로 분할한 후 $K$ 번 복원추출하여 재구성한 훈련용 데이터 세트
    ...
    - 데이터 세트 $$x \in \mathcal{D}$$ 를 $$k$$ 개의 데이터 블록 $$\Lambda_{1}, \cdots, \Lambda_{K}$$ 으로 나누자
...
- **OOB(`O`ut `o`f `B`ag)** : 부트스트랩에 포함되지 않는 데이터 블록 집합으로서 검증용 데이터 세트로 활용됨
    ...
    - $m$ 번째 모형의 검증용 데이터 세트는 해당 모형의 부트스트랩에 포함되지 않은 데이터 블록으로 구성함
        ...
        $$\begin{aligned}
        \Omega^{(m)}_{\text{val}}
        &= \bigcup_{k=1}^{K}{\Lambda_{k}} \setminus \Omega^{(m)}_{\text{trn}}
        \end{aligned}$$

    - 하나의 데이터 블록이 부트스트랩에 포함되지 않을 확률은 다음과 같음

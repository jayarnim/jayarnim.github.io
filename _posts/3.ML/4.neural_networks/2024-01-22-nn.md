...
image:
    path: /assets/img/posts/3.ML/4.neural_networks/Thumbnail.jpg
---
...
- **퍼셉트론(Perceptron)** : 다수의 신호를 입력 받아 하나의 신호를 출력하는 알고리즘
    ...
    ![03](/assets/img/posts/3.ML/4.neural_networks/01-03.png){: width="100%"}
...
- **논리 회로(Logic Gate)** : 하나 이상의 논리 입력을 받아 일정한 논리 연산을 거쳐 논리 출력을 얻는 회로
    ...
    ![05](/assets/img/posts/3.ML/4.neural_networks/01-05.jpg){: width="100%"}
...
- **MLP(`M`ulti-`L`ayer `P`erceptron)**
    ...
    ![07](/assets/img/posts/3.ML/4.neural_networks/01-07.png){: width="100%"}

    - 단층 퍼셉트론은 선형 분류기로서 게이트를 다양한 가중치 조합을 통해 표현할 수 있으나, 비선형 분류기를 요하는 일부 게이트(`XOR`, `XNOR`)에 대해서는 표현이 불가능함

        ![01](/assets/img/posts/3.ML/4.neural_networks/01-01.jpg){: width="100%"}

    - 이는 `NAND`, `OR`, `AND` 게이트를 표현하는 퍼셉트론들의 조합으로 표현할 수 있음

        ![02](/assets/img/posts/3.ML/4.neural_networks/01-02.png){: width="100%"}
...
- **인공신경망(Artifical `N`eural `N`etwork)** : 생물학적 신경망에서 영감을 얻은 통계학적 학습 알고리즘
    ...
    ![06](/assets/img/posts/3.ML/4.neural_networks/01-06.png){: width="100%"}

- **MLP(Multi-Layer Perceptron) is FC(`F`ully-`C`onnected) FFNN(`F`eed-`F`orward `N`eural `N`etwork)**

    ![08](/assets/img/posts/3.ML/4.neural_networks/01-08.png){: width="100%"}
...
- **MLP Definition**
    ...
    ![09](/assets/img/posts/3.ML/4.neural_networks/01-09.png){: width="100%"}
...
- **활성화 함수(Activation Function)** : 다음 레이어 뉴런으로의 전달 여부를 판단하는 함수($h(\cdot)$)
    ...
    ![04](/assets/img/posts/3.ML/4.neural_networks/01-04.png){: width="100%"}
...
- **그라디언트(Gradient)** : 다변수 함수에 대하여 모든 방향으로의 순간변화율 벡터
    ...
    ![11](/assets/img/posts/3.ML/4.neural_networks/01-11.png){: width="100%"}
...
- **경사하강법(Gradient Descent)** : 손실 함수의 도함수(그라디언트)를 최소화하는 가중치를 추정하는 방법
    ...
    ![12](/assets/img/posts/3.ML/4.neural_networks/01-12.png){: width="100%"}
...
- **역전파(Backward Propagation)** : 경사하강법을 활용하여 오차를 출력층에서 입력층 방향으로 전파하며 가중치를 조정하는 학습 방법
    ...
    ![10](/assets/img/posts/3.ML/4.neural_networks/01-10.png){: width="100%"}
...
- **Update Learning Rate-based Approach**
    ...
    ![13](/assets/img/posts/3.ML/4.neural_networks/01-13.png){: width="100%"}
...
- **Update Learning Direction-based Approach**
    ...
    ![14](/assets/img/posts/3.ML/4.neural_networks/01-14.png){: width="100%"}

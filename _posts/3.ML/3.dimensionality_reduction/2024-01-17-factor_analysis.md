...
- **요인 분석(Factor Analysis)**: 기존에는 관찰되지 않았으나 의미 있는 소수의 핵심 요인(Latent Factor)으로 측정 변수들을 요약하여 차원을 축소하는 방법
...
- 목적 함수(완전 데이터 로그 우도):
    ...
    $$\begin{aligned}
    \log{p(X\mid\Lambda,\Psi)}
    :=\log{\int_{i}{p(x_{i},f_{i}\mid\Lambda,\Psi)}{\mathrm{d}F}}
    \end{aligned}$$

- 잠재요인 $F$ 의 사후 확률 분포 $q$ 정의:

    $$\begin{aligned}
    q(F)
    :=p(F\mid X,\Lambda,\Psi)
    \end{aligned}$$
...
- 목적 함수의 기대값 변환:
    ...
    $$\begin{aligned}
    \log{p(X\mid\Lambda,\Psi)}
    &=\log{\int_{i}{\frac{p(x_{i},f_{i}\mid\Lambda,\Psi)}{q(f_{i})}\cdot q(f_{i})}{\mathrm{d}F}}\\
    &=\log{\mathbb{E}_{q(F)}\left[\frac{p(X,F\mid\Lambda,\Psi)}{q(F)}\right]}
    \end{aligned}$$

- 젠센 부등식에 의하여 좌변의 하한값을 Q 함수로 정리할 수 있음:

    $$\begin{aligned}
    \log{\mathbb{E}_{q(F)}\left[\frac{p(X,F\mid\Lambda,\Psi)}{q(F)}\right]}
    &\ge\mathbb{E}_{q(F)}\left[\log{\frac{p(X,F\mid\Lambda,\Psi)}{q(F)}}\right]\\
    &=\underbrace{\mathbb{E}_{q(F)}\left[\log{p(X,F\mid\Lambda,\Psi)}\right]}_{=:Q(\Lambda,\Psi)}-\underbrace{\mathbb{E}_{q(F)}\left[\log{q(F)}\right]}_{=:H(q)}
    \end{aligned}$$
...
- 잠재요인의 사후 확률 분포:
    ...
    q(F)
    :&=p(F\mid X,\Lambda,\Psi)\\
    &\propto p(X\mid F,\Lambda,\Psi)p(F)
    \end{aligned}$$

- prior of latent factor:

    $$\begin{aligned}
    p(F)
    =\mathcal{N}(0,\mathbf{I})\quad\because\mathrm{def}
    \end{aligned}$$

- likelihood:

    $$\begin{aligned}
    p(X\mid F,\Lambda,\Psi)
    =\mathcal{N}(\Lambda F,\Psi)\quad\because F,\Lambda,U\sim\mathcal{N}
    \end{aligned}$$
...
- definition:
    ...
    $$\begin{aligned}
    M
    &:=\mathbb{E}\left[F\mid X\right]\in\mathbb{R}^{N\times K}\\
    R
    &:=\mathbb{E}\left[F^{T}F\mid X\right]\in\mathbb{R}^{K\times K}
    \end{aligned}$$
...
- lambda update:
    ...
    $$\begin{aligned}
    \Lambda^{*}
    &=X^{T}M R^{-1}\in\mathbb{R}^{P\times K}
    \end{aligned}$$
...
- psi update:
    ...
    $$\begin{aligned}
    \Psi^{*}
    &=\mathrm{diag}\left[\frac{1}{n}\left(X-M\left(\Lambda^{*}\right)^{T}\right)^{T}\left(X-M\left(\Lambda^{*}\right)^{T}\right)\right]\in\mathbb{R}^{P\times K}
    \end{aligned}$$

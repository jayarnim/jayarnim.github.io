...
- **Architecture**
    ...
    - **디코더(Decoder)** : 문맥 벡터를 참조하여 목표 언어의 단어들을 순차로 출력하는 모듈(NLG Process)
        ...
        - $$\sigma_{t}$$ : $t$ 시점 셀 상태
        - $$\hat{\mathbf{y}}_{t}$$ : 목표 언어의 $t$ 번째 단어 확률 분포 벡터
...
- **어텐션 메커니즘(Attention Application)** : RNN 계열 레이어의 기울기 소실 문제를 보완하는 기법으로서, 문맥 벡터를 최종 은닉 상태로 활용하지 않고, 인코더 각 순번 은닉 상태와 디코더 현재 시점 간 관련성을 고려하여, 디코더의 각 시점에 특화된 문맥 벡터를 생성함
    - **루옹 어텐션(Luong Attention)** : 디코더의 현재 시점 은닉 상태를 활용하여 문맥 벡터를 생성하는 어텐션 기법
    - **바다나우 어텐션(Bahdanau Attention)** : 디코더의 이전 시점 은닉 상태를 활용하여 문맥 벡터를 생성하는 어텐션 기법
...
- **주의 가중치(Attention Wieght) 도출 함수 정의**
    ...
    | General | $$f(\overrightarrow{\mathbf{q}}, \mathbf{K}) = \overrightarrow{\mathbf{q}}^{T} \cdot \mathbf{W} \cdot \mathbf{K}$$ | Luong et al. (2015) |
...
![03](/_post_refer_img/TextAnalytics/08-04.png){: width="100%"}

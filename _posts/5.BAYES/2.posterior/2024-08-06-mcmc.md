...
- **마르코프 체인 몬테 카를로(`M`arkov `C`hain `M`onte `C`arlo)** : 이전 단계에서 추출한 표본을 기반으로 다음 단계의 표본을 순차로 추출하여 사후 분포에 근사하는 방법
    ...
    ![01](/_post_refer_img/5.BAYES/2.posterior/02-01.png){: width="100%"}
...
- **제안 분포(Proposal Dist.)** : 시점 $t$ 에서 수락된 파라미터 샘플 $\theta^{(t)}$ 에 기반하여 다음 시점 $t+1$ 에서 샘플링 위치 $\psi$ 를 제안하는 분포
    ...
    - $\sigma^2$ 의 크기와 샘플링 수렴 여부의 관계:
        ...
        ![02](/_post_refer_img/5.BAYES/2.posterior/02-02.png){: width="100%"}
...
- **자기상관(Auto-Correlation)** : 순차로 발생한 일련의 관측치 $\{x^{(t)} \mid t\text{ is time point}\}$ 간에 존재하는 상관관계
    ...
    ![03](/_post_refer_img/5.BAYES/2.posterior/02-03.png){: width="100%"}
...
- **자기상관계수(Auto-Correlation Coefficient)** : 순서에 의미가 있는 데이터에서, 현재 시점의 값 $x^{(t)}$ 과 그 과거 또는 미래의 값 $x^{(t-k)}$ 간의 상관관계를 측정하는 지표
    ...
    - $k$ : 시간 간격(Lag)
        ...
        ![04](/_post_refer_img/5.BAYES/2.posterior/02-04.png){: width="100%"}

- **솎아내기(Thinning)** : 매 $k$ 번째 표본을 선택함으로써 자기상관을 줄이는 방법

    ![05](/_post_refer_img/5.BAYES/2.posterior/02-05.png){: width="100%"}

    - 통상 $k \le 10$ 으로 설정함

- **선행 구간(Burn-in Period)** : 수렴 상태에 도달하기 전, 초기값 $x^{(t=0)}$ 의 영향력을 최소화하기 위해 일부 반복을 무시하는 구간

    ![06](/_post_refer_img/5.BAYES/2.posterior/02-06.png){: width="100%"}
